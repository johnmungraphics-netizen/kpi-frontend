...
const ManagerKPIDetails: React.FC = () => {
  ...
  const [textModal, setTextModal] = useState<{ isOpen: boolean; title: string; value: string; field?: string; itemId?: number }>({
    isOpen: false,
    title: '',
    value: '',
  });
  ...
  const fetchKPI = async () => {
    try {
      ...
      // Ensure we only show items for this specific KPI form
      // Filter items to ensure they belong to this kpi_id (backend should already do this, but double-check)
      if (kpiData.items && Array.isArray(kpiData.items)) {
        kpiData.items = kpiData.items.filter((item: any) => {
          // Items should already be filtered by backend, but ensure consistency
          return item.kpi_id === parseInt(kpiId!) || !item.kpi_id; // Allow items without kpi_id for legacy data
        });
      }
      
      setKpi(kpiData);
      
      // Find review for this specific KPI (not all reviews for the employee)
    ...
    <div className="space-y-6">
                    ...
                    kpi.items.map((item, index) => {
                      ...
                      return (
                        <tr key={item.id}>
                          ...
                          <td className="px-6 py-4">
                            <button
                              onClick={() => setTextModal({ isOpen: true, title: 'KPI Title', value: item.title || 'N/A', field: 'title', itemId: item.id })}
                          ...
                          <td className="px-6 py-4">
                            <button
                              onClick={() => setTextModal({ isOpen: true, title: 'KPI Description', value: item.description || 'N/A', field: 'description', itemId: item.id })}
                          ...
                          <td className="px-6 py-4">
                            <button
                              onClick={() => setTextModal({ isOpen: true, title: 'Current Performance Status', value: item.current_performance_status || 'N/A', field: 'current_performance_status', itemId: item.id })}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={() => navigate(`/manager/employee-kpis/${kpi.employee_id}`)}
            className="flex items-center space-x-2 px-4 py-2 text-purple-600 hover:text-purple-700"
          >
            <FiUser className="text-lg" />
            <span>View All {kpi.employee_name}'s KPIs</span>
          </button>
        </div>
        <div className="flex items-center space-x-3">
